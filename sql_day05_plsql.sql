...
-- PROCEDURE를 DBMS에선 컴파일 하여 저장하고 사용하는 객체

-- TABLE, SEQUENCE, INDEX, VIEW 등과 같은 객체 이므로
...
-- EXECUTE 프로시저 이름(전달할값1, 전달할값2) : SQL*PLUS 명령
...
IS
    -- 지역 변수 선언 블록, BEGIN ~ END 사이에서 사용되는 변수 선언부
    -- 변수타입 선언시 크기까지 지정
